# Basisleistungen

| BEB97 | Leistung | Menge | Bemerkung |
|--------|--------|--------|--------|
| 0021 | Modell für Sägesegmente | 1 | Arbeitsmodell (Sägemodell) |
| 0002 | Modell aus Superhartgips | 1 | Gegenkiefermodell |
| 0103 | Modellsegment sägen | 2 | je Sägeschnitt |
| 0104 | Stumpf aus Superhartgips | 1 | je Stumpf |
| 0404 | Modellmontage in individuellen Artikulator I | 1 | Modellpaar in einem Arbeitsgang |
| 2122 | Krone gegossen, für Keramik- oder Polymer-Glas-Vollverblendung | je Krone | NEM-Gerüst gegossen; Gerüstposition der modularen Kette |
| 2612 | Mehrflächige Verblendung aus Keramik | je Krone | keramische Vollverblendung |
| 2981 | NEM Zuschlag | je Krone | Verarbeitungszuschlag NEM; alternativ zu 2982 |
| 0701 | Versand je Versandgang | 2 | je Einlieferung/Abholung (BEB-Zählweise, abweichend von BEL 9330) |

# Zusatzleistungen

| BEB97 | Leistung | Menge | Bemerkung |
|--------|--------|--------|--------|
| 0007 | Kontrollmodell | 1 | bei zusätzlichem Kontroll-/Teilmodell |
| 0019 | Frässockel | 1 | bei Frässockel-Technik |
| 0111 | Modellpaar trimmen | 1 | bei gesondertem Trimmen |
| 0114 | Zweitstumpf aus Superhartgips | je Stumpf | bei Zweit-/Kontrollstumpf |
| 2124 | Stufenkrone gegossen, für Keramik- oder Polymer-Glas-Vollverblendung | je Krone | alternativ zu 2122 bei Stufenpräparation |
| 2162 | Krone gefräst, für Keramik- oder Polymer-Glas-Vollverblendung | je Krone | statt 2122 bei CAD-CAM-gefrästem Gerüst |
| 2982 | Sonderlegierung verarbeiten | je Krone | statt 2981 bei nicht betriebsüblicher Legierung |
| 2611 | Teilverblendung aus Keramik | je Krone | alternativ zu 2612 bei Teilverblendung |
| 2616 | Keramikverblendung in Mehrschichttechnik | je Krone | alternativ zu 2612 bei Layering-/Schichttechnik |
| 2605 | Vollverblendung Komposite | je Krone | alternativ zu 2612 bei Kompositverblendung |
| 2675 | Schulter aus Keramik/Glas | je Krone | bei zervikaler Keramikschulter |
| 2676 | Papille aus Keramik/Glas | je Interdentalraum | bei Papillenersatz |
| 2677 | Zahnfleisch aus Keramik/Glas | je Zahn | bei Gingivaersatz keramisch |
| 2951 | Individuell charakterisieren, Keramik | je Krone | bei individueller Charakterisierung |
| 2952 | Zuschlag bei Verarbeitung von Spezialkeramik | je Krone | bei nicht standardkonformer Keramikmasse |
| 2955 | Glasieren, je Einheit | je Krone | bei Glanzbrand |
| 2959 | Mehraufwand durch Rohbrandeinprobe | je Krone | bei Rohbrandeinprobe |
| 2963 | Mehrere Farben in Kronen- oder Brückenverband | je Zahnfarbe | bei mehrfarbiger Ausführung (Ausführungsaufwand, nicht Farbbestimmung) |
| 2689 | Farbgebung durch Bemalen | je Krone | alternativ zu 2963 bei Maltechnik statt Schichtung |
| 2840 | Endkontrolle unter Stereomikroskop | je Krone | bei mikroskopischer Endkontrolle |
| 2965 | Zuschlag für Arbeiten unter Stereomikroskop | je Krone | alternativ zu 2840 |
| 2801 | Kaufläche nach gnathologischen Kriterien gestaltet, in Metall/gegossenem Glas | je Kaufläche | bei gnathologischer Kauflächengestaltung in Metall |
| 2802 | Kaufläche nach gnathologischen Kriterien gestaltet, in Keramik | je Kaufläche | bei gnathologischer Kauflächengestaltung in Keramik |
| 2803 | Frontzahn nach gnathologischen Kriterien gestaltet, in Metall/gegossenem Glas | je Zahn | bei gnathologischer Frontzahngestaltung in Metall |
| 2804 | Frontzahn nach gnathologischen Kriterien gestaltet, in Keramik | je Zahn | bei gnathologischer Frontzahngestaltung in Keramik |
| 2811 | Selektives Einschleifen nach Remontage/Krone, Brückenglied, Inlay | je Krone | bei Remontage/okklusaler Korrektur |
| 2915 | Okklusaler Stop | je Stop | bei Okklusionssicherung |
| 2911 | Krone, Brückenglied für Klammer vorbereiten | je Krone | bei Kombination mit herausnehmbarem ZE; Rastenlager 3024 nicht enthalten |
| 2912 | Krone, Brückenglied, Inlay passend für vorhandene Prothese arbeiten | je Krone | bei vorhandener Prothese |
| 2922 | Krone/Inlay/Brückenglied aufpassen | je Krone | bei Aufpassen auf Modell |
| 5301 | Deckgold aufbrennen | je Krone | bei Oxidabdeckung vor Keramikbrand |
| 5302 | Bonder aufbrennen | je Krone | bei Bonder-Vorbehandlung Metall/Keramik |
| 5306 | Keramik/gegossenes Glas konditionieren | je Krone | bei adhäsiver Befestigung |
| 5307 | Metallfläche konditionieren | je Krone | nur bei Kunststoff-/Kompositverblendung (2605) |
| 0911 | Foto auf Bildbearbeitungssoftware matchen | 1 | bei digitaler Farb-/Formabstimmung |
| 0912 | Foto-/Videodokumentation zur diagnostischen Auswertung | 1 | bei diagnostischer Dokumentation |
| 0723 | Zahnfarbenbestimmung I | 1 | bei Verblendung/zahnfarbener Arbeit, je Fall |
| 0724 | Zahnfarbenbestimmung II | 1 | bei Sonderfarbe ohne Konfektionsfarbmuster |
| 0732 | Desinfektion | 2 | Eingangs- + Ausgangsdesinfektion |
| 0710 | Eilterminzuschlag | 1 | bei urgency=urgent/express |
| 0706 | Foto- oder Video-Dokumentation | 1 | bei Dokumentationsbedarf |
| 0721 | Zeiteinheit; Zahntechniker-Meister | je | bei Sonderaufwand, je angefangene Zeiteinheit |
| 0722 | Zeiteinheit; Zahntechniker | je | bei Sonderaufwand, je angefangene Zeiteinheit |
| 0702 | Sonderversand oder Fahrtkosten | 1 | bei Express-/Sonderlogistik |

# Material

| Material | Menge | Bemerkung |
|--------|--------|--------|
| NEM-Legierung (CoCr) | ca. 4-6 g | gesondert: Verbrauch in g; Bezeichnung, Menge und Preis auf der Laborrechnung ausweisen (§ 10 Abs. 2 Nr. 5 GOZ) |

# Hinweise

- Modularer Weg 2122 (Gerüst) + 2612 (Verblendung); nicht mit den Komplettpositionen 2807/2810/2829 kombinieren
- 2981 (NEM) und 2982 (Sonderlegierung) sind Zuschlagsalternativen — nie beide je Krone; 0721/0722 nicht neben ausgewiesenen Einzelleistungen desselben Arbeitsschritts
- 0701 zählt je Einlieferung/Abholung (Standardfall 2), BEL 9330 dagegen je Hin- und Rückweg — Zählweisen nicht vermischen
- → Vorlage ../gleichartig/VMK-NEM_voll bei GKV-Patient mit HKP (dann gleichartig mit Festzuschuss 1.1, +1.3 im Verblendbereich OK 15-25/UK 34-44); diese Vorlage nur für PKV bzw. ausdrücklichen Komplett-Privatwunsch ohne HKP
- Im Leistungspreis enthalten: Verblendkeramik (Schichtmassen), Modellgips (Hartgips), Superhartgips (Stumpfmaterial), Einbettmasse, Konditionierer/Bonder, Isolier-/Trennmittel
- VERIFIZIEREN: Hauslogik des Modellblocks (0021 Sägemodell + 0002 Gegenkiefer + 0103 je Sägeschnitt + 0104 je Stumpf); 0007 nur bei tatsächlichem Kontrollmodell
